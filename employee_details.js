const employees = [
  ...
  // function desplay employees
/*const div = document.getElementById('employeesDetails');
...
function findBySpecailizaion(specialization) {
    const foundSp = employees.find(employee => employees.specialization === specialization );
    if(foundSp){
        document.getElementById('employeesDetails').innerHTML = `<p>${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.name} - ${foundEmployee.department} - $${foundEmployee.salary}</p>`;
